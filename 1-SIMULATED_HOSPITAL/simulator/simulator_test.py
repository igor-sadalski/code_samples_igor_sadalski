...
def to_mllp(segments):
    ...
    m += bytes(chr(simulator.MLLP_END_OF_BLOCK) +
               chr(simulator.MLLP_CARRIAGE_RETURN), "ascii")
    return m
...
class SimulatorTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(wait_until_healthy(
            self.simulator, f"localhost:{TEST_PAGER_PORT}"))
    ...
    def test_page_with_valid_mrn(self):
        ...
        r = urllib.request.urlopen(
            f"http://localhost:{TEST_PAGER_PORT}/page", data=mrn)
